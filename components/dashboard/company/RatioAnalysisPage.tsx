...
/**
 * RatioAnalysisPage — Ratio Analysis tab.
 * 
...
interface RatioAnalysisPageProps { ticker: string; companyName: string }
...
const ratioData = {
  RELIANCE: {
    ...
    profitability: [
      { id: "gross-margin", title: "Gross Margin", value: "33.8%", status: "Fair" },
      { id: "ebitda-margin", title: "EBITDA Margin", value: "22.4%", status: "Fair" },
      { id: "net-margin", title: "Net Margin", value: "8.5%", status: "Fair" },
      { id: "roe", title: "ROE", value: "9.8%", status: "Fair" },
    ],
    liquidity: [
      { id: "current-ratio", title: "Current Ratio", value: "1.10x", status: "Fair" },
      { id: "quick-ratio", title: "Quick Ratio", value: "0.82x", status: "Poor" },
    ],
    leverage: [
      { id: "debt-equity", title: "Debt/Equity", value: "0.60x", status: "Good" },
      { id: "interest-cover", title: "Interest Cover", value: "8.2x", status: "Good" },
    ],
  },
};
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {ratios.map((r) => (
          <div key={r.id} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-5 backdrop-blur-xl transition-colors duration-300 hover:border-white/[0.12]">
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <button className="text-xs text-slate-500 transition-colors hover:text-cyan-400">View Trend</button>
            </div>
          </div>
        ))}
      </div>
...
export default function RatioAnalysisPage({ ticker, companyName }: RatioAnalysisPageProps) {
  const data = (ratioData as any)[ticker] || ratioData.RELIANCE;
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-6 backdrop-blur-xl">
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.revenueNetProfit} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
              <XAxis dataKey="year" tick={{ fill: "#64748b", fontSize: 11 }} tickLine={false} axisLine={{ stroke: "rgba(255,255,255,0.06)" }} />
              <YAxis tick={{ fill: "#64748b", fontSize: 10 }} tickLine={false} axisLine={{ stroke: "rgba(255,255,255,0.06)" }} tickFormatter={(v) => `₹${(v / 1000).toFixed(0)}K`} />
              ...
              <Legend wrapperStyle={{ fontSize: "12px", color: "#94a3b8" }} formatter={(v) => v === "revenue" ? "Revenue" : "Net Profit"} />
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-6 backdrop-blur-xl">
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data.marginTrends} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
              <XAxis dataKey="year" tick={{ fill: "#64748b", fontSize: 11 }} tickLine={false} axisLine={{ stroke: "rgba(255,255,255,0.06)" }} />
              <YAxis tick={{ fill: "#64748b", fontSize: 10 }} tickLine={false} axisLine={{ stroke: "rgba(255,255,255,0.06)" }} tickFormatter={(v) => `${v}%`} />
              <Tooltip contentStyle={{ background: "rgba(15,23,42,0.95)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", color: "#fff", fontSize: "12px" }} />
              <Legend wrapperStyle={{ fontSize: "12px" }} formatter={(v) => v === "grossMargin" ? "Gross Margin" : v === "operatingMargin" ? "Operating Margin" : "Net Margin"} />
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-6 backdrop-blur-xl">
        ...
        <p className="mb-4 text-xs text-slate-500">Multi-dimensional financial assessment</p>
        <div className="mx-auto h-80 max-w-md">
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart cx="50%" cy="50%" outerRadius="75%" data={data.radarAxes}>
              ...
              <Radar name="Score" dataKey="value" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.15} strokeWidth={2} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
      </div>
